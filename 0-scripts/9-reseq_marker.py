...
reseqs = ['Cbp', 'Cor', 'Cru']
for r in reseqs:
    reseq_mat = pd.read_csv(f'reseq_{r}.mat', sep='\t', dtype=str)
    if r == 'Cbp':
        reseq_Cor_mat = reseq_mat.iloc[:98357, :]
        reseq_Cru_mat = reseq_mat.iloc[98357:, :]
        ...
        for t in types_cor_list:
            ...
            allfilt_reseq_mat.to_csv(f'reseq_{r}.{t}.marker', sep='\t', index=None)
            print(f'reseq_{r}.{t}.marker done')
