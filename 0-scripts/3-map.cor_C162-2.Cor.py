...
pop_mat = pd.read_csv(f'matrix_pop/panTE.cor.matrix', sep='\t', dtype=str, keep_default_na=False)
Cru_mat = pd.read_csv(f'matrix_19/panTE.C162-2.Cor.matrix', sep='\t', dtype=str, keep_default_na=False)
...
no_ref_merge = pop_merge_df[pop_merge_df.iloc[:, 8:].isnull().all(axis=1)]
ref_merge_remain = pop_merge_df[pop_merge_df.iloc[:, 5:8].isnull().all(axis=1)]
...
merge_df1.to_csv(f'/data2/share/home/liyi/CbpTE/ReseqTE_matrix/19Ge_reseq_matrix/int_file/panTE.cor_C162-2.Cor.map', sep='\t', index=None)